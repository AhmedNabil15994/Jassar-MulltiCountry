...
<head>
    ...
    @endif
    <link rel="icon"
        href="{{ setting('images.favicon') ? url(setting('images.favicon')) : url('frontend/favicon.png') }}" />
</head>

<body>
    <div class="page-content"
        style="background: url({{ setting('images.landing_background') ?? url('frontend/landing/images/bg.png') }}); background-size: cover; background-repeat: no-repeat;">
        ...
        <div class="container position-relative">
            ...
            <a class="logo d-block text-center" href="{{ route('frontend.home') }}"><img class="img-fluid"
                    src="{{ setting('images.logo') ? url(setting('images.logo')) : url('frontend/images/header-logo.png') }}"
                    alt="" /></a>
            @if (setting('app_name.' . locale()))
                <div class="row">
            ...
            <div class="row app-content">
                <div class="col-md-7">
                    <div class="app-description text-center">
                        @if (setting('app_description.' . locale()))
                            <p>
                                {!! setting('app_description.' . locale()) !!}
                            </p>
                        @endif

                        @if (setting('about_app.android_download_url') || setting('about_app.ios_download_url'))
                            <div class="app-download">
                                <h2>{{ __('apps::frontend.download.title') }}</h2>
                                <div class=" d-flex justify-content-center">

                                    @if (setting('about_app.android_download_url'))
                                        <a class="d-flex align-items-center download-btn"
                                            href="{{ setting('about_app.android_download_url') ?? '#' }}">
                                    ...
                                    @if (setting('about_app.ios_download_url'))
                                        <a class="d-flex align-items-center download-btn"
                                            href="{{ setting('about_app.ios_download_url') ?? '#' }}">
                        ...
                        <div class="d-flex align-items-center justify-content-center share-buttons">
                            @if (setting('social.facebook'))
                                <a class="facebook" href="{{ setting('social.facebook') ?? '#' }}">
                                    <span class="fab fa-facebook-f"></span>
                                </a>
                            @endif

                            @if (setting('social.twitter'))
                                <a class="twitter" href="{{ setting('social.twitter') ?? '#' }}">
                            ...
                            @if (setting('social.snapchat'))
                                <a class="rounded-circle snapchat"
                                    href="{{ setting('social.snapchat') ?? '#' }}">
                ...
                <div class="col-md-5 position-relative">
                    @if (!empty(setting('app_gallery')))
                        <div class="app-screens">
                            
                            <div class="screens owl-carousel">
                                @foreach (setting('app_gallery') as $image)
                                    <div class="item">
                                        <img class="img-fluid"
        ...
        {{-- <div class="phone-call cbh-phone cbh-green cbh-show  cbh-static" id="clbh_phone_div">
            <a target="_blank" id="WhatsApp-button"
                href="https://wa.me/{{ setting('contact_us.whatsapp') }}?text={{ __('apps::frontend.general.how_can_we_help') }}"
        ...
        <a href="https://wa.me/{{ setting('contact_us.whatsapp') }}?text={{ __('apps::frontend.general.how_can_we_help') }}"
            target="_blank">
