...
trait ShoppingCartTrait
{
    ...
    public function validateProduct($product,$request,$max=0){
        $checkQty = $this->checkQty($product);
        $vendorStatus = $this->vendorStatus($product);
        $checkMaxQty = $this->checkMaxQty($product, $request,$max);
    ...
    public function addOrUpdateCart($product, $request,$type='product')
    {
        ...
        if($type == 'product'){
            $validateProduct = $this->validateProduct($product,$request);
            if($validateProduct){
            ...
            foreach($product->products as $oneProduct){
                $oneProduct->product_type = 'package';
                $validateProduct = $this->validateProduct($oneProduct,$request);
                if($validateProduct){
                    return $validateProduct;
                }
            }
            ...
            foreach($product->products as $oneProduct){
                $oneProduct->product_type = 'offer';
//                $oneProduct->qty = $product->qty;
                $validateProduct = $this->validateProduct($oneProduct,$request,$product->qty);
                if($validateProduct){
                    return $validateProduct;
                }
            }

            foreach($product->freeProducts as $prod){
                $prod->product_type = 'offer';
                $max = 0;
                foreach($product->selectedProducts as $item){
                    if($item['product_id'] == $prod->id){
                        $max = $item['qty'];
                    }
                }
                $validateProduct = $this->validateProduct($prod,$request,$max);
                if($validateProduct){
                    return $validateProduct;
                }
            }
    ...
    {
        $productTitle = $product->product_type == 'product' ? $product->title : ($product->product_type == 'package' ? $product->title : $product->title);
    ...
    public function checkMaxQty($product, $request,$max=0)
    {
        $request->qty = $max ? $max : $request->qty;
        $productTitle = $product->product_type == 'product' ? $product->title :  ($product->product_type == 'package' ? $product->title : $product->title);
    ...
    */
    public function vendorStatus($product)
    {
        ...
        if ($vendor) {
            ...
            if ($vendor->vendor_status_id == 4 || (setting('other.select_shipping_provider') == 'vendor_delivery' && !$this->isAvailableVendorWorkTime($vendor->id))) {
    ...
    */
    ...
    public function checkMaxQtyInCheckout($product, $itemQty, $cartQty)
    {
        if ($product && !is_null($product->qty)) {
            ...
            if ($itemQty > $product->qty)
                return __('catalog::frontend.products.alerts.qty_more_than_max') . ' ' . $cartQty . ' - ' . $product->title;
        }
        return false;
    }
    ...
    {
        $item = $this->findItemById($product->product_type == 'product' ? $product->id : ($product->product_type == 'package' ? 'var-package-' . $product->id : 'var-offer-' . $product->id));
        if (!is_null($item)) {
    ...
    public function add($product, $request)
    {
        ...
        if($product->product_type == 'package'){
            ...
            $attributes['slug'] = $productName;
            $attributes['secondary_title']   = $product->secondary_title;

            $cartArr = [
                'id' =>  'var-package-' . $product->id,
                'name' => $productName,
                'quantity' => 1,
            ...
            $cartArr['price'] = number_format($cartArr['price'],3);
        }else if($product->product_type == 'offer'){
            $attributes['image'] =  $product->image;
            $attributes['sku'] =  $product->id;
            $productName = $product->title;
            ...
            $cartArr = [
                'id' =>  'var-offer-' . $product->id,
                'name' => $productName,
                'quantity' => 1,
                'attributes' => $attributes,
            ];
            $cartArr['selectedProducts'] = $product->selectedProducts;
            $cartArr['price'] = floatval($product->price);
            $cartArr['price'] = number_format($cartArr['price'],3);
        }else{
            $attributes['image'] =  $product->getFirstMediaUrl('images');
    ...
    public function updateCart($product, $request)
    {
        ...
        if (isset($product->product_type) && $product->product_type == 'product') {
            $attributes['image'] = $product->image;
            $attributes['sku'] = $product->sku;
            $productName = $product->title;
            $attributes['slug'] = $product->slug;
            ...
            $cartArr['price'] = $cartArr['price'] + $addonsOptionsTotal;
        }else if (isset($product->product_type) && $product->product_type == 'offer'){
            $cartArr = [
        ...
        if (auth()->check())
            $updateItem = Cart::session(auth()->user()->id)->update($product->product_type == 'product' ? $product->id : ($product->product_type == 'package' ? 'var-package-' : 'var-offer-') . $product->id, $cartArr);
        else {
            if (is_null(get_cookie_value(config('core.config.constants.CART_KEY')))) {
                $cartKey = Str::random(30);
                set_cookie_value(config('core.config.constants.CART_KEY'), $cartKey);
            ...
            $updateItem = Cart::session($cartKey)->update($product->product_type == 'product' ? $product->id : ($product->product_type == 'package' ? 'var-package-' : 'var-offer-') . $product->id, $cartArr);
        }
    ...
    public function companyDeliveryChargeCondition($request, $price, $userToken = null, $delivery_time = null, $cityId = null, $countryId = null)
    {
        $deliveryFees = new CartCondition([
    ...
    public function removeCartItemsCouponValue($userToken = null)
    {
        $items = getCartContent($userToken);
        if (!$items->isEmpty()) {
            foreach ($items as $item) {
                Cart::removeItemCondition($item->id, 'product_coupon');
            }
        }
    }

    public function removeCartCoupons($userToken = null)
    {
        $userCartToken = $userToken ?? $this->getCartUserToken();
        $this->removeCartConditionByType('coupon_discount',$userCartToken);
        $this->removeCartItemsCouponValue($userCartToken);
        return true;
    }
