...
if (!function_exists('getCountryCode')) {
    ...
    {
        $basePref = 'kw';
        $userPref = request()->segment(2);
        if(auth()->check()){
            $prefArr = auth()->user()->countries()->pluck('iso2')->toArray();
            if(count($prefArr)){
                $userPref = in_array(strtoupper($userPref),$prefArr) ? $userPref : $prefArr[0];
                $countryObj = Country::active()->where('iso2','LIKE','%'.$userPref.'%')->first();
            }else{
                $countryObj = Country::find(auth()->user()->setting->countries[0] ?? 118);
            }

            $basePref = strtolower($countryObj->iso2);
        }else{
             $ip = request()->ip() == '127.0.0.1' ? '156.219.252.55' : request()->ip();
             $position = \Location::get($ip);
             $basePref = strtolower($position->countryCode);
        }
        return $basePref;
    }
}
...
if (!function_exists('is_rtl')) {
    ...
    function is_rtl($locale = null)
    {
        ...
        $rtlLocales = is_null(Setting::get('rtl_locales')) ? [] : Setting::get('rtl_locales');

        if (in_array($locale, $rtlLocales) && active_menu('headermenus') != 'active') {
            return 'rtl';
        }
...
if (!function_exists('toggleSideMenuItemsByVendorType')) {
    ...
    {
        return setting('other.is_multi_vendors') == 1 ? 'block' : 'none';
    }
}
...
if (!function_exists('getCart')) {
    function getCart($userToken = null, $force = false, $forceUpdateCurrency=false)
    {
        $cartContent = getCartContent($userToken, $force);
        $getCartCouponDiscount = getCartCouponDiscount();
        $subTotal = getCartSubTotal($userToken);
        $total = getCartTotal($userToken);

         return [
            'items' => array_values(CartResource::collection($cartContent)->jsonSerialize()),
            'original_cart_subTotal' => priceWithCurrenciesCode($subTotal,true,false),
            'original_cart_total' => priceWithCurrenciesCode($total,true,false),
            'cart_subTotal' => $forceUpdateCurrency ? priceWithCurrenciesCode($subTotal,true,false) : priceWithCurrenciesCode($subTotal),
            'cart_total' => $forceUpdateCurrency ? priceWithCurrenciesCode($total,true,false) : priceWithCurrenciesCode($total),
            'conditions' => [
                'coupon' => $getCartCouponDiscount ? ($forceUpdateCurrency ? priceWithCurrenciesCode($getCartCouponDiscount,true,false) :  priceWithCurrenciesCode($getCartCouponDiscount)) : 0
            ]
        ];
    }
}

if (!function_exists('getCartCouponDiscount')) {
    function getCartCouponDiscount()
    {
        $coupon = Cart::getCondition('coupon_discount');

        if($coupon && $coupon->getValue()){
            return number_format(abs($coupon->getValue()), 3);
        }

        $coupon = getCartItemsCouponValue();

        if($coupon){
            return number_format($coupon, 3);
        }
...
if (!function_exists('getOrderShipping')) {
    function getOrderShipping($userToken = null)
    {
        ...
        if (!is_null($userToken))
            $result = Cart::session($userToken)->getCondition('company_delivery_fees') != null ? Cart::session($userToken)->getCondition('company_delivery_fees')->getValue() : null;
        else
...
if (!function_exists('checkActivePayment')) {
    function checkActivePayment()
    {
        ...
        foreach (array_keys(setting('supported_payments')) ?? [] as $key => $payment) {
            if (setting('supported_payments.' . $payment . '.status') == 'on') {
...
if (!function_exists('getDukaanProductPrice')) {
    function getDukaanProductPrice($model,$data)
    {
        if(isset($data['product_flag']) && $data['product_flag'] != 'single')
        {
            $data['price'] = "<div class=\"price\">".__('apps::frontend.master.price_on_selection')."</div>";
            $data['has_offer'] = false;
            return $data;
        }

        $price = priceWithCurrenciesCode($model->price);

        if($model->offer){
            $offerPrice = !is_null($model->offer->offer_price) ? $model->offer->offer_price : calculateOfferAmountByPercentage($model->price, $model->offer->percentage);

            $offerPrice = priceWithCurrenciesCode($offerPrice);
            $data['price'] = "<div class=\"price\">{$offerPrice} <del class=\"discount\">{$price}</del></div>";
            $data['offer_percentage'] = !is_null($model->offer->offer_price) ? calculateOfferPercentageByAmount($model->price, $model->offer->offer_price) : number_format($model->offer->percentage,2);
            $data['has_offer'] = true;
        }else{
            $data['price'] = "<div class=\"price\">{$price}</div>";
            $data['has_offer'] = false;
        }
...
if (!function_exists('currentCountry')) {
    ...
    {
        if (session()->has('current_country') && in_array(session()->has('current_country'),(array)Setting::get('supported_countries'))) {
            return session()->get('current_country');
        }

        return Setting::get('default_country');
    }
}

if (!function_exists('refreshCurrency')) {
    function refreshCurrency($code = null)
    {
        $tenantSubdomain = app('currentTenant')->subdomain;

        if($code || !Cache::has("{$tenantSubdomain}_currency_data")){

            $code = $code ?? currentCurrencyCode();
            ...
            if ($currency) {
                info('_currency_data changed');
                Cache::forget("{$tenantSubdomain}_currency_data");
                Cache::put("{$tenantSubdomain}_currency_data",[
...
if (!function_exists('currentCurrencyCode')) {
    function currentCurrencyCode($currency_data = null)
    {
        if(!$currency_data){
            $tenantSubdomain = app('currentTenant')->subdomain;
            $currency_data = Cache::get("{$tenantSubdomain}_currency_data");
            if (
                $currency_data && count($currency_data) &&
                isset($currency_data['selected_currency']) &&
                isset($currency_data['currencies_value'])
            ) {
                return $currency_data['selected_currency'];
            }
            return  CURRENCY ?? Setting::get('default_currency');

        }else{
            return isset($currency_data['selected_currency']) ? $currency_data['selected_currency'] : Setting::get('default_currency');
        }
    }
}

if (!function_exists('priceWithCurrenciesCode')) {
    function priceWithCurrenciesCode($price, $price_return = true, $currency_code_return = true, $currency_data = null)
    {
        $code = currentCurrencyCode($currency_data);
        ...
        if(!$currency_data){
            ...
            if (
                ...
                $price = number_format($price * $currency_data['currencies_value'], 2);
                $code = $currency_data['selected_currency'];
            }
        }else{

            if (isset($currency_data['selected_currency']) && isset($currency_data['currencies_value'])) {
        ...
        return ($price_return ? $price : '') .($currency_code_return && $price_return ? ' ' : ''). ($currency_code_return ? $code : '');
    }
}
