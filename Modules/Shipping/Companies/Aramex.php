...
class Aramex implements ShippingTransactionsInterface
{
    ...
    public function createShipment(Request $request, Order $order): void
    {
        ...
        if($consigneeAddress){
            ...
            ->setTransportType(0)
            ->setPickupLocation(setting('contact_us.address'))
    ...
    private function getShipperContact(): object
    {
        return (new Contact())
        ->setDepartment('5')
        ->setPersonName(setting('app_name.' . locale()))
        ->setCompanyName(setting('app_name.' . locale()))
        ->setPhoneNumber1(setting('contact_us.mobile'))
        ->setCellPhone(setting('contact_us.whatsapp'))
        ->setEmailAddress(setting('contact_us.email'));
    }
}
