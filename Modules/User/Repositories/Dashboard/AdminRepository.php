...
class AdminRepository
{
    ...
    public function create($request)
    {
        ...
        try {
            ...
            $data = [
                ...
                "setting" => [
                    "lang" => locale(),
                    'countries' => $request['setting']['countries'] ?? null,
                ],
    ...
    */
    ...
    {
        $query = $this->user->with('roles')->whereNotIn('id', [1, auth()->id()])->whereHas('roles.perms', function ($query) {
            $query->where('name', 'dashboard_access');
        })->where('is_internal', '!=', 1)->where(function ($query) use ($request) {
            ...
            $query->orWhereHas('roles', function ($query) use ($request) {
                $query->where('display_name', 'like', '%' . $request->input('search.value') . '%');
            });
        });
