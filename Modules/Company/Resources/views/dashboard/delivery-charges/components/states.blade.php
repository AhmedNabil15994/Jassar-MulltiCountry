<div class="col-md-12">
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-hover">
            ...
            <th>
                <span>{{__('company::dashboard.delivery_charges.update.min_order_for_free_delivery')}}</span>
                <div class="pull-right" title="{{__('company::dashboard.delivery_charges.update.btn.activate_all')}}">
                    <button type="button" class="btn btn-success btn-sm pull-left"
                            onclick="copyToAllInputValues('min_order_for_free_delivery','min-order-for-free-delivery-input-{{$city->id}}')"
                            title="{{__('company::dashboard.delivery_charges.update.btn.copy')}}">
                        <i class="fa fa-copy"></i>
                    </button>
                    <input type="checkbox" class="make-switch makeAllActiveCheckbox" onchange="makeAllActiveCheckbox(this, 'min-order-for-free-delivery-input-{{$city->id}}')"
            ...
            @foreach ($city->states as $state)
                <tr>
                    ...
                    <td>
                        <div class="col-xs-8">
                            <input type="text" name="min_order_for_free_delivery[{{$state->id}}]"
                                   class="form-control min-order-for-free-delivery-input min-order-for-free-delivery-input-{{$city->id}}"
                                   value="{{!array_key_exists($state->id, $min_order_for_free_delivery) ? "" : $min_order_for_free_delivery[$state->id]}}"
                                   placeholder="{{__('company::dashboard.delivery_charges.update.min_order_for_free_delivery')}}" style="visibility: {{!array_key_exists($state->id, $min_order_for_free_delivery) ? "hidden" : 'show'}}">
                        </div>
                        <div class="col-xs-4">
                            <input type="checkbox" class="make-switch min-order-for-free-delivery-input min-order-for-free-delivery-input-{{$city->id}}"
                               data-size="small" onchange="freeDelivery(this)" {{!array_key_exists($state->id, $min_order_for_free_delivery) ? "" : 'checked'}}>
                        </div>
                    </td>
                    <td>
