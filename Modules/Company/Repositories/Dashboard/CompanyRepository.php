    ...
    public function create($request)
    {
        ...
        try {
            $company = $this->company->create([
                ...
                'image' => $request->image ? Attachment::addAttachment($request['image'],'companies') : url(setting('logo')),
           ...
           if($request->countries){
            ...
            if ($request->delivery_time_types && in_array('direct', $request->delivery_time_types ?? [])) {
                $company->direct_delivery_message = $request->direct_delivery_message ?? null;
            } else {
                $company->direct_delivery_message = null;
            }
            $company->save();
...
//            }
    ...
    {
        $query = CompanyCountry::with(['company','country']);
            if(!auth()->user()->is_internal){
                $query = $query->whereIn('country_id',auth()->user()->setting->countries ?? []);
            }else{
                $query = $query->whereIn('country_id',setting('supported_countries'));
            }
