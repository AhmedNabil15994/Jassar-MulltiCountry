...
class CreateOrderRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        ...
        $validator->after(function ($validator) use ($userToken) {
            ...
            } elseif (setting('other.select_shipping_provider') == 'vendor_delivery') {
            ...
            if ($shippingType == 'schedule') {
                ...
                $this->request->add(['shipping' => ['message' => $deliveryProvider->direct_delivery_message]]);
            }
            ...
            if (!in_array($this->payment, array_keys(setting('supported_payments') ?? []) ?? []) || setting('supported_payments.' . $this->payment . '.status') != 'on') {
                return $validator->errors()->add(
                    'payment',
