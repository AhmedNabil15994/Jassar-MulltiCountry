...
trait OrderCalculationTrait
{
    public function calculateTheOrder($userToken = null)
    {
        ...
        $order['shipping_details'] = getCartConditionByName($userToken, 'company_delivery_fees') ? getCartConditionByName($userToken, 'company_delivery_fees')->getAttributes() : [];
        $order['total'] = $total;

        $productsCollection = collect($order["products"]);
        $productsCollection = $productsCollection->groupBy("vendor_id");
    ...
    public function orderProducts($userToken = null)
    {
        ...
        foreach (getCartContent($userToken) as $k => $value) {
            ...
            if ($value->attributes->product_type == 'product') {
            ...
            } else if($value->attributes->product_type == 'package') {
                $vendor = $value->attributes->product->products[0]->vendor;
                $product['product_type'] = 'package';
                $product['vendor_id'] = $vendor->id;
                $productObject = Package::find($value->attributes->product->id);
                ...
                $product['vendor_id'] = $vendor->id;
                $productObject = Offer::find($value->attributes->product->id);
                $product['selectedProducts'] = $value->attributes->product->selectedProducts;
            ...
            // Check Product Offer
            if (!is_null($productObject) && $value->attributes->product_type == 'product' ) {
            ...
            } else if(!is_null($productObject) && $value->attributes->product_type == 'package') {
                $offerPrice = $productObject->price;
            }
